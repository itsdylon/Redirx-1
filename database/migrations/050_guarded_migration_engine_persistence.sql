...
BEGIN
 SELECT id,user_id,status,locked_by,attempt_count,lease_expires_at,mcp_run_id
 INTO s.id,s.user_id,s.status,s.locked_by,s.attempt_count,s.lease_expires_at,s.mcp_run_id
 FROM migration_sessions WHERE id=p_session_id AND mcp_run_id=p_run_id FOR UPDATE;
...
-- Membership follows the exact immutable run snapshot, using 032's inventory
-- URL index. Do not repeatedly copy or linearly scan 35k queued JSON URLs.
...
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path=public,pg_temp AS $$
 SELECT EXISTS(SELECT 1 FROM migration_runs r JOIN session_discovered_urls u
   ...
   WHERE r.id=p_run_id AND u.side=p_side AND u.url=p_url)
$$;
...
BEGIN
 SELECT id,mcp_run_id INTO s.id,s.mcp_run_id FROM migration_sessions WHERE id=NEW.session_id;
 ...
 IF TG_TABLE_NAME='webpage_embeddings' THEN
  IF NEW.site_type NOT IN ('old','new') OR NOT migration_engine_url_belongs(s.mcp_run_id,NEW.site_type,NEW.url) THEN
   RAISE EXCEPTION 'invalid_input' USING ERRCODE='P0001'; END IF;
 ELSE
  IF NOT migration_engine_url_belongs(s.mcp_run_id,'old',NEW.old_url) OR (NEW.new_url IS NOT NULL AND NOT migration_engine_url_belongs(s.mcp_run_id,'new',NEW.new_url)) THEN
   RAISE EXCEPTION 'invalid_input' USING ERRCODE='P0001'; END IF;
 END IF;
 RETURN NEW;
END $$;
...
BEGIN
 ...
 IF p_site_type NOT IN ('old','new') OR p_url IS NULL OR NOT migration_engine_url_belongs(p_run_id,p_site_type,p_url)
...
BEGIN
 s:=lock_migration_engine_attempt(p_session_id,p_run_id,p_worker_id,p_attempt_count);
 IF p_old_url IS NULL OR NOT migration_engine_url_belongs(p_run_id,'old',p_old_url) OR (p_new_url IS NOT NULL AND NOT migration_engine_url_belongs(p_run_id,'new',p_new_url))
 ...
 RETURN jsonb_build_object('id',found_id,'replayed',false);
END $$;
REVOKE ALL ON FUNCTION lock_migration_engine_attempt(uuid,uuid,text,integer),migration_engine_url_belongs(uuid,text,text),guard_migration_engine_insert(),
